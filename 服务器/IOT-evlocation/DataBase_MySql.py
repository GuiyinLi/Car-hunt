...
'''
# 加入日志
logging.basicConfig(filename='/home/evlocation/log/flask.log',
                    format='%(asctime)s %(levelname)-7s:%(message)s', level=logging.INFO, filemode='w', datefmt='%Y-%m-%d %H:%M:%S')
# 获取logger实例
...
class DB_MySql():
    ...
    def connectDatabase(self, user, password, db, host='localhost', port=3306, charset='utf8'):
        ...
        self.__conInfo['port'] = port  # 3306
        self.__conInfo['user'] = user  # root
        self.__conInfo['password'] = password  # 170707109
        self.__conInfo['db'] = db  # library management system
